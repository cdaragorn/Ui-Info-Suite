...
namespace UIInfoSuite.UIElements {
	class LocationOfTownsfolk: IDisposable {
		...
		private readonly IDictionary<String, String> _townspeopleOptions;
		...
		public LocationOfTownsfolk(ModOptions modOptions, ModConfig config, IModHelper helper) {
			_helper = helper;
			_townspeopleOptions = config.Townspeople;
		...
		private void OnMenuChange(object sender, EventArgsClickableMenuChanged e) {
			if (Game1.activeClickableMenu is GameMenu) {
				...
				foreach (var friendName in _friendNames) {
					...
					checkbox.isChecked = _townspeopleOptions.SafeGet(hashCode.ToString()).SafeParseBool();
				}
			}
		}
			...
			if (Game1.activeClickableMenu is GameMenu) {
				...
				for (int i = slotPosition; i < slotPosition + 5; ++i) {
					...
					if (checkbox.bounds.Contains(Game1.getMouseX(), Game1.getMouseY()) &&
						...
						_townspeopleOptions[checkbox.whichOption.ToString()] = checkbox.isChecked.ToString();
				...
				if (gameMenu.currentTab == 3) {
					foreach (var character in _townsfolk) {
						try {
							...
							bool drawCharacter = _townspeopleOptions.SafeGet(hashCode.ToString()).SafeParseBool();
